...
local mini_icon = maker.image(icons.user, colors.transparent, 4, 50, "icon_user")

local notification_count_widget = wibox.widget {
    widget = wibox.widget.textbox,
    markup = maker.text(colors.fg, " Regular 10", "0")
}
...
local function reset_notification_widgets()
    ...
    notification_count_widget.markup = maker.text(colors.fg, " Regular 10", "0")
    notification_app_msg_widget.markup = maker.text(colors.fg, " Regular 9", "Remetente")
...
awesome.connect_signal("theme::colors", function(colors)
    notification_app_name_widget:set_markup(maker.text(colors.fg, " Regular 10", " Notificações"))
    notification_app_msg_widget:set_markup(maker.text(colors.fg, " Regular 9", "Remetente"))
    notification_count_widget:set_markup(maker.text(colors.fg, " Regular 10", tostring(notification_count)))
end)
