...
local function set_wallpaper(image_path)
    ...
    for s in screen do apply_wallpaper(s) end
...
image_container:connect_signal("button::press", function(_, _, _, button)
    if button == 4 then
        start_image = start_image - 1
        if start_image < 1 then start_image = #images end
        update_images()
    elseif button == 5 then
        start_image = start_image + 1
        if start_image > #images then start_image = 1 end
        update_images()
    end
end)

local wall = maker.image(icons.wallpaper, colors.transparent, 4, 0, "image_icon")

awesome.connect_signal("theme::icons", function(icons)
    wall:get_children_by_id("image_icon")[1]:set_image(icons.wallpaper)
end)

wall:buttons(gears.table.join(awful.button({}, 1, nil, function()
...
local widgets_right = { maker.margins(main_papers, 0, 0, 2, 2) }
