...
local menu_bnt = maker.image(icons.menu, colors.transparent, dpi(6), dpi(0), "menu_bnt")
-- Criar um botão para a informação
local info_bnt = maker.image(icons.infos, colors.transparent, dpi(6), dpi(0), "info_bnt")
-- Criar um botão para o modo
local mode_bnt = maker.image(update_mode_icon(), colors.transparent, dpi(6), dpi(0), "mode_bnt")
-- Criar um botão para o menu
...
info_bnt:buttons({                                -- Botões do botão de informação
    awful.button(nil, 1, function()               -- Botão esquerdo
        if not info_center.visible then           -- Se o centro de informação não estiver visível
            ...
            gears.timer.start_new(0.1, function() -- Iniciar um timer para fechar o centro de informação
