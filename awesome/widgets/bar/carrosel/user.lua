...
local wm_name = wibox.widget({
    ...
    valign = "center",
    markup = maker.text(colors.fg, " Regular 10", settings.wm_name)
})

local user_name = wibox.widget({
    widget = wibox.widget.textbox,
    valign = "center",
    markup = maker.text(colors.fg, " Regular 9", "@" .. settings.name)
})
...
awesome.connect_signal("theme::colors", function(colors)
    ...
    user_name:set_markup(maker.text(colors.fg, " Regular 9",
        "@" .. settings.name))
end)
return main_user
