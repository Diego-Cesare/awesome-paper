...
maker.hover(power, colors.transparent, nil, 1)     -- Adicionar efeito de hover

local reboot = maker.button(icons.reboot, 50, nil) -- Criar um botão para o reboot
maker.hover(reboot, colors.transparent, nil, 1)    -- Adicionar efeito de hover

local exit = maker.button(icons.exit, 50, nil)     -- Criar um botão para o exit
maker.hover(exit, colors.transparent, nil, 1)      -- Adicionar efeito de hover
...
power.forced_width = dpi(50)                       -- Definir a altura do botão
reboot.forced_width = dpi(50)                      -- Definir a altura do botão
exit.forced_width = dpi(50)                        -- Definir a altura do botão
...
local usericon = wibox.widget({              -- Criar um widget para o ícone do usuário
    ...
    border_width = settings.elements_border, -- Tamanho do borda
    id = "user_icon",
    border_color = colors.fg,                -- Cor do borda
...
local main_power = wibox.widget({           -- Criar um widget para o botão de power
    ...
    maker.margins(power_box, 20, 0, 10, 0), -- Adicionar margens
})

-- Criar um widget para o nome do host
local awesomeWM = wibox.widget.textbox()
-- Adicionar o texto ao widget
awesomeWM.markup = maker.text(colors.fg .. "99", settings.font .. " Regular 30", "AwesomeWM")
...
local wm_box = wibox.widget({                 -- Criar um widget para o nome do WM
    ...
    maker.margins(usericon, 80, 0, 50, 0),    -- Adicionar margens
})

menu.main_widget = wibox.widget({ -- Criar um widget para o menu
    layout = wibox.layout.fixed.vertical,
    forced_height = dpi(650),     -- Altura do widget
    ...
    },
    {
        ...
        {
            widget = wibox.container.background, -- Widget para exibir o menu
            maker.margins(wm_box, 20, 0, 20, 0), -- Adicionar margens
        },
        {
            ...
            margins = { top = dpi(20), bottom = dpi(20), left = dpi(10), right = dpi(10) }, -- Margens
            {
                ...
                border_width = dpi(1),                                                      -- Tamanho do borda
                border_color = colors.fg,                                                   -- Cor do borda
...
function menu:add_entries() -- Função para adicionar as entradas ao menu
    ...
    for i, entry in ipairs(self.filtered) do      -- Adicionar as entradas ao objeto
        ...
        if i == self.index_entry then            -- Se a entrada for a atual
            ...
            entry_widget.border_width = dpi(1)
            entry_widget.border_color = colors.fg
...
awesome.connect_signal("change::theme", function(c)                                     -- Quando o tema mudar
    ...
    menu.popup_widget.border_color = colors.fg
    ...
    awesomeWM.markup = maker.text(colors.fg .. "99", settings.font .. " Regular 30", "AwesomeWM")
    distro.markup = maker.text(colors.fg, settings.font .. " Regular 20", settings.host)
    usericon:get_children_by_id("user_icon")[1].border_color = colors.fg
end)

awesome.connect_signal("theme::icons", function(icons)
    power.image = icons.power
    reboot.image = icons.reboot
    exit.image = icons.exit
end)
